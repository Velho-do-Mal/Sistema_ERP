...
_NEON_URL = (
    "postgresql://neondb_owner:npg_pfeYTX50bOaz@ep-fragrant-cherry-a4staimy-pooler.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require"
)
...
def get_finance_db() -> Tuple[object, object]:
    ...
    Prioridade: env DATABASE_URL -> st.secrets -> URL Neon embutida.
    Sempre usa Neon (PostgreSQL) — sem fallback SQLite.
    """
    ...
    if not db_url:
        try:
            ...
            db_url = str(general.get("database_url", "")).strip() if isinstance(general, dict) else ""
        except Exception:
            db_url = ""
    if not db_url:
        db_url = _NEON_URL

    # Garante que o bk_finance.get_db() encontre a URL

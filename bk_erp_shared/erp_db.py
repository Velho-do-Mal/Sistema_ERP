...
def get_finance_db() -> Tuple[object, object]:
    ...
    Esta versão é defensiva para ambiente Streamlit Cloud / secrets:
    - Prioriza a variável de ambiente os.environ['DATABASE_URL'] se existir.
    - Caso não exista, procura em st.secrets['DATABASE_URL'].
    - Caso ainda não exista, procura em st.secrets['general']['database_url'].
    - Se encontrar em st.secrets, grava em os.environ['DATABASE_URL'] para compatibilidade
      com bk_finance.get_db() que pode ler a variável de ambiente.
    - Finalmente, chama bk_finance.get_db() e retorna o (engine, SessionLocal).
    """
    # 1) Prioriza variável de ambiente existente
    db_url = os.environ.get("DATABASE_URL")
    if not db_url:
        # 2) Verifica st.secrets: primeiro chave de nível superior
        try:
            db_url = st.secrets.get("DATABASE_URL")
        except Exception:
            db_url = None
    if not db_url:
        # 3) Verifica seção 'general' (se você usar .streamlit/secrets.toml com [general])
        try:
    ...
    if db_url:
        # garante que o bk_finance.get_db() encontre a URL no ambiente
        os.environ["DATABASE_URL"] = db_url

    # Finalmente, retorna o par do módulo financeiro (bk_finance.get_db faz o create_engine e sessionmaker)
